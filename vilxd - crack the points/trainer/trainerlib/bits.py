"""
Pure-Python helper functions (portable).

These are small arithmetic/bit-manipulation helpers used by various crackme scripts.
"""


def print_hex(value: int) -> str:
    """
    Return `value` formatted as uppercase hex.

    - If `value` is an int: returns `0x...` (like `hex(value).upper()`).
    - If `value` is bytes/bytearray: returns space-separated hex bytes.
    """
...
def neg(value: int, width: int = 32) -> int:
    ...
    _mask = (1 << width) - 1
    value &= _mask
    return (-value) & _mask  # same as (0 - value) & mask
...
def rol(value: int, shift: int, width: int = 32) -> int:
    ...
    value &= _mask
    return ((value << shift) | (value >> (width - shift))) & _mask


# Backwards-compatible name (older scripts)
phex = print_hex
